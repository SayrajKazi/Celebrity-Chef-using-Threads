...
public :
  ...
  void		serve		(Chef&		chef,
      ...
      while(dishPtr_!= NULL){
        pthread_mutex_lock(&countLock_);
        std::cout<< "WOULD YOU CLEAR THAT stinky DISH OFF THE TABLE!" << std::endl;
        pthread_mutex_unlock(&countLock_);
...
void* cook (void* vPtr)
{
  ...
  {
    Dish* returnDish = chefPtr->prepare();
    table.serve(*chefPtr,returnDish);
  }
  delete (chefPtr);
  return NULL ;
			...
			)
{
  ...
  for (int i=0; i < NUM_DISHES_TO_DO; i++ )
  {
    Dish *toeat= table.eatFrom(*gourmandPtr);

    gourmandPtr->consume(toeat);
			...
			)
{
  ...
  for (int i = 0; i < NUM_CHEFS; i++)
  {
    ...
    for (int i = 0; i < NUM_CHEFS; i++)
  {
      Gourmand* gourmand = new Gourmand(i);
      pthread_create(&gourmandIds[i],NULL,eat,gourmand);
  }
